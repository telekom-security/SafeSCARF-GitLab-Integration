...
def iter_vulns(element):
    ...
    finding = {
        ...
        "numerical_severity": "{}".format(element["severity_index"]),
        "cve": element["cve"],
    }
    return finding
...
def iter_components(component, result_list, finding):
    ...
    finding_by_artefact["component_name"] = component["name"]
    finding_by_artefact["component_version"] = component["version"]
...
def parse_data(input):
    ...
    PARSED_DATA["tags"] = input["tags"]
    PARSED_DATA["sla_start_date"] = input["started_at"][:10]
    PARSED_DATA["sla_expiration_date"] = input["finished_at"][:10]

    # Each individual Vulnerability
    for element in input["vulnerabilities"]:
        finding = iter_vulns(element)
        # Artifacts for each individual vulnerability
        for component in element["artifacts"]:
